...
export default function AdminPanel() {
  ...
  const [editing, setEditing] = useState(null); // product being edited, or "new" for new product
  ...
  // Check auth by trying to load products
  const login = async () => {
    ...
    try {
      ...
      // If it was a real product added, delete it - but actually we'll just validate
      setAuthed(true);
      loadProducts();
    } catch {
      // If the API doesn't exist yet (first deploy), just let them in
      setAuthed(true);
      loadProducts();
    }
  };

  // Start new product form
  const startNew = () => {
    setEditing("new");
    setForm({
      name: "",
      sku: "QP" + Math.floor(10000 + Math.random() * 90000),
      price: "",
      category: CATEGORIES[1] || "Exotic",
      image: "",
      inStock: true,
      description: "",
      smellRating: "",
    ...
    try {
      if (editing === "new") {
        ...
        const res = await fetch("/api/products", {
          method: "PUT",
          headers,
          body: JSON.stringify({ id: editing, ...productData }),
        });
        if (res.ok) {
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ background: "var(--card)", borderRadius: 20, border: "1px solid var(--border)", padding: 40, maxWidth: 400, width: "100%" }}>
          ...
          <input
            ...
            style={{ ...inputStyle, marginBottom: 16, padding: "14px 18px", fontSize: 16 }}
            autoFocus
          />
          <button onClick={login} style={{ ...btnStyle(), width: "100%", padding: "14px", fontSize: 16 }}>
            🔓 Login
          </button>
          {message && (
            <p style={{ marginTop: 12, textAlign: "center", fontSize: 13, color: message.type === "error" ? "var(--red)" : "var(--green)" }}>
              {message.text}
            </p>
          )}
        </div>
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <header style={{ background: "rgba(10,10,15,.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid var(--border)", padding: "0 24px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <a href="/" style={{ fontSize: 13, color: "var(--muted)" }}>← View Store</a>
      ...
      {message && (
        <div style={{
          ...
          background: message.type === "error" ? "var(--red)" : "var(--green)",
          color: "#fff", padding: "10px 24px", borderRadius: 10, fontSize: 14, fontWeight: 600,
          boxShadow: "0 8px 32px rgba(0,0,0,.3)", animation: "fadeUp .3s ease",
        }}>
          {message.text}
        </div>
      )}
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 24px 80px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {products.map((product) => (
                <div key={product.id} style={{
                  ...
                  background: "var(--card)", borderRadius: 14, padding: "14px 20px",
                  ...
                  <div style={{ flex: 1, minWidth: 150 }}>
                    ...
                    <p style={{ fontSize: 11, color: "var(--dim)" }}>
                      ...
                      {product.inStock === false && <span style={{ color: "var(--red)", marginLeft: 8 }}>● Out of Stock</span>}
                    </p>
                  </div>

                  {/* Price */}
                  <span style={{ fontFamily: "'Outfit'", fontWeight: 700, fontSize: 18, color: "var(--accent)", minWidth: 100, textAlign: "right" }}>
                    {fmt(product.price)}
                  </span>
        ...
        {editing && (
          <div style={{ maxWidth: 700, margin: "0 auto" }}>
            <button onClick={() => setEditing(null)} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, marginBottom: 20 }}>
              ← Back to Products
            </button>

            <h2 style={{ fontFamily: "'Outfit'", fontSize: 24, fontWeight: 700, marginBottom: 24 }}>
            ...
            <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", padding: 24 }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                {form.image && (
                  <div style={{ marginBottom: 12, borderRadius: 12, overflow: "hidden", maxHeight: 250 }}>
                    <img src={form.image} alt="Preview" style={{ width: "100%", maxHeight: 250, objectFit: "cover" }} />
                  </div>
                )}
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => fileRef.current.click()} disabled={uploading}
                    style={{ ...btnStyle("var(--surface)"), border: "1px solid var(--border)", color: "var(--accent)" }}>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 24, paddingTop: 20, borderTop: "1px solid var(--border)" }}>
                <button onClick={saveProduct} style={{ ...btnStyle(), flex: 1, padding: "14px", fontSize: 15 }}>
                  {editing === "new" ? "✓ Create Product" : "✓ Save Changes"}
                </button>
                <button onClick={() => setEditing(null)} style={{ ...btnStyle("var(--surface)"), border: "1px solid var(--border)", color: "var(--muted)", padding: "14px 24px" }}>
                  Cancel
                </button>
              </div>
            </div>
