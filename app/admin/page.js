...
export default function AdminPanel() {
  ...
  const genMsg = (f, isNew) => {
    var fire = String.fromCodePoint(0x1F525);
    var plane = String.fromCodePoint(0x2708, 0xFE0F);
    var pkg = String.fromCodePoint(0x1F4E6);
    var bang = String.fromCodePoint(0x203C, 0xFE0F);
    var star = String.fromCodePoint(0x2B50, 0xFE0F);
    var point = String.fromCodePoint(0x1F449, 0x1F3FC);
    var url = "https://astro448.app";
    var nm = f.name || "Product";
    var sk = f.sku || "";
    var pr = f.salePrice || f.price || "0";
    var sr = parseInt(f.smellRating) || 0;
    var bdg = f.badge || "";
    var hdr = isNew ? fire + " NEW DROP " + fire : fire + " Product Updated " + fire;
    var lines = [hdr, "<b>" + nm + "</b>", "SKU: " + sk];
    if (bdg && bdg !== "NEW") lines.push(bdg);
    if (sr > 0) lines.push("Nose: " + star.repeat(sr));
    lines.push("<b>$" + pr + "</b>" + plane);
    if (f.noDiscount) { lines.push("No Additional Discounts" + bang); } else { lines.push("<b>($" + (Math.max(0, parseInt(pr) - 200)) + ")</b> 5+ " + pkg); }
    lines.push(point + " " + url);
    return lines.join("\n");
  };
  ...
  const getNextSku = () => {
    const nums = products.map(p => { const m = (p.sku||"").match(/^AP(\d+)$/); return m ? parseInt(m[1]) : 0; });
    const max = nums.length > 0 ? Math.max(...nums) : 0;
    return "AP" + String(max + 1).padStart(5, "0");
  };

  const startNew = () => {
    const f = { name:"", sku:getNextSku(), price:"", category:CATEGORIES[1]||"Highs", image:"", video:"", inStock:true, description:"", smellRating:"", strain:"", weight:"", badge:"NEW", dateAdded:todayStr(), dateUpdated:"", salePrice:"", noDiscount:false, _internalDateAdded:new Date().toISOString() };
    f.telegramMessage = genMsg(f, true);
    setEditing("new");
    setForm(f);
  };

  const startEdit = (p) => {
    const f = {...p, telegramMessage: genMsg(p, false)};
    setEditing(p.id);
    setForm(f);
  };
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      if (editing==="new") { await fetch("/api/products",{method:"POST",headers:hd,body:JSON.stringify(pd)}); flash("Product saved!"); }
      else { await fetch("/api/products",{method:"PUT",headers:hd,body:JSON.stringify({id:editing,...pd})}); flash("Product saved!"); }
  ...
  const saveAndAnnounce = async () => {
    if (!form.name||!form.price) { flash("Name and price required","error"); return; }
    const pd = {...form, price:parseInt(form.price)||0, salePrice:form.salePrice?parseInt(form.salePrice):null, noDiscount:form.noDiscount||false, inStock:form.inStock!==false};
    const tgMsg = form.telegramMessage || genMsg(form, editing === "new");
    delete pd.telegramMessage;
    if (editing==="new" && !pd._internalDateAdded) pd._internalDateAdded = new Date().toISOString();
    try {
      const hd = {"x-admin-password":password,"Content-Type":"application/json"};
      if (editing==="new") { await fetch("/api/products",{method:"POST",headers:hd,body:JSON.stringify(pd)}); }
      else { await fetch("/api/products",{method:"PUT",headers:hd,body:JSON.stringify({id:editing,...pd})}); }
      try {
        const nr = await fetch("/api/notify",{method:"POST",headers:hd,body:JSON.stringify({message:tgMsg})});
        const nd = await nr.json();
        if (nd.sent) flash("Saved + Telegram sent!");
        else if (nd.reason==="missing_env_vars") flash("Saved. Telegram skipped (env vars not set)","error");
        else flash("Saved. Telegram failed: "+(nd.reason||""),"error");
      } catch { flash("Saved, Telegram request failed","error"); }
      setEditing(null); loadProducts();
    } catch { flash("Save failed","error"); }
  };
  ...
  const quickOOS = async (id) => {
    try {
      await fetch("/api/products",{method:"PUT",headers:{"x-admin-password":password,"Content-Type":"application/json"},body:JSON.stringify({id,inStock:false})});
      flash("Marked out of stock");
      loadProducts();
      ...
      <header style={{background:"rgba(10,10,15,.88)",backdropFilter:"blur(20px)",borderBottom:"1px solid var(--border)",padding:"0 24px",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:64}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>setAdminView("products")} style={{background:adminView==="products"?"var(--card)":"transparent",border:"1px solid "+(adminView==="products"?"var(--border)":"transparent"),color:adminView==="products"?"var(--text)":"var(--muted)",padding:"7px 14px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:500}}>Products</button>
            <button onClick={()=>setAdminView("announcements")} style={{background:adminView==="announcements"?"var(--card)":"transparent",border:"1px solid "+(adminView==="announcements"?"var(--border)":"transparent"),color:adminView==="announcements"?"var(--text)":"var(--muted)",padding:"7px 14px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:500}}>Announcements</button>
            <a href="/shop" style={{fontSize:13,color:"var(--muted)",marginLeft:8}}>Store</a>
            <a href="/admin/settings" style={{fontSize:13,color:"var(--accent)"}}>Settings</a>
            <button onClick={()=>{setAuthed(false);setPassword("");}} style={{...bs("var(--surface)"),border:"1px solid var(--border)",color:"var(--muted)"}}>Logout</button>
          </div>
        </div>
      </header>
      ...
      <main style={{maxWidth:1100,margin:"0 auto",padding:"24px 24px 80px"}}>
            ...
            {products.filter(p=>p.name).map(product=>(
              <div key={product.id} style={{display:"flex",alignItems:"center",gap:16,background:"var(--card)",borderRadius:14,padding:"14px 20px",border:"1px solid var(--border)",flexWrap:"wrap"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  {product.inStock!==false&&<button onClick={()=>quickOOS(product.id)} style={{...bs("var(--surface)"),border:"1px solid var(--border)",color:"var(--red)",fontSize:12,opacity:0.8}}>Out of Stock</button>}
        ...
        {adminView==="products"&&editing&&<div style={{maxWidth:700,margin:"0 auto"}}>
          ...
          <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24}}>
            ...
            {/* ── TELEGRAM ANNOUNCEMENT PREVIEW ── */}
            <div style={{marginTop:24,paddingTop:20,borderTop:"1px solid var(--border)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <label style={{fontSize:13,fontWeight:600,color:"var(--muted)",textTransform:"uppercase"}}>Telegram Announcement Preview</label>
                <button onClick={()=>setForm(p=>({...p,telegramMessage:genMsg(p,editing==="new")}))} style={{background:"none",border:"1px solid var(--border)",color:"var(--accent)",fontSize:11,fontWeight:600,padding:"4px 12px",borderRadius:6,cursor:"pointer"}}>Regenerate</button>
              </div>
              <textarea value={form.telegramMessage||""} onChange={e=>setForm(p=>({...p,telegramMessage:e.target.value}))} rows={8} style={{...is,resize:"vertical",fontFamily:"monospace",fontSize:13,lineHeight:1.5}}/>
              <p style={{fontSize:11,color:"var(--dim)",marginTop:6,fontStyle:"italic"}}>This preview is editable. Modify the text above to customize the Telegram announcement. Click &quot;Regenerate&quot; to reset from product details. Click &quot;Save and Announce&quot; to send.</p>
            </div>

            {/* ── SAVE BUTTONS ── */}
            <div style={{display:"flex",gap:12,marginTop:24,paddingTop:20,borderTop:"1px solid var(--border)"}}>
              <button onClick={saveProduct} style={{...bs("var(--surface)"),border:"1px solid var(--border)",color:"var(--text)",flex:1,padding:"14px",fontSize:15}}>{editing==="new"?"Create Product":"Save Changes"}</button>
              <button onClick={saveAndAnnounce} style={{...bs(),flex:1,padding:"14px",fontSize:15,background:"linear-gradient(135deg,#0088cc,#0077b5)"}}>{editing==="new"?"Create & Announce":"Save & Announce"}</button>
          ...
          {!annEditing?<>
            ...
            <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24,display:"grid",gap:16}}>
              ...
              <div style={{paddingTop:20,borderTop:"1px solid var(--border)"}}>
                <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:12}}><input type="checkbox" checked={!!annForm.sendTelegram} onChange={e=>{const ck=e.target.checked;setAnnForm(p=>{const msg=String.fromCodePoint(0x1F4E2)+" "+(p.title||"Announcement")+"\n\n"+(p.body||"");return{...p,sendTelegram:ck,telegramMessage:ck?(p.telegramMessage||msg):p.telegramMessage};});}} style={{accentColor:"var(--accent)",width:18,height:18}}/><span style={{fontSize:14,fontWeight:600}}>Send Telegram announcement on save</span></label>
