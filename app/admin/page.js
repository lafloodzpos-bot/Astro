          ...
          <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24}}>
            ...
            <div style={{marginTop:24,paddingTop:20,borderTop:"1px solid var(--border)"}}>
              <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:12}}>
                <input type="checkbox" checked={!!form.sendTelegram} onChange={e=>{const ck=e.target.checked;setForm(p=>{const msg=editing==="new"?"\u{1F195} NEW DROP\n\n"+(p.name||"Product")+"\nSKU: "+(p.sku||"")+"\nPrice: $"+(p.price||"0"):"\u{1F504} UPDATE\n\n"+(p.name||"Product")+"\nSKU: "+(p.sku||"")+"\nPrice: $"+(p.price||"0");return{...p,sendTelegram:ck,telegramMessage:ck?(p.telegramMessage||msg):p.telegramMessage};});}} style={{accentColor:"var(--accent)",width:18,height:18}}/>
                <span style={{fontSize:14,fontWeight:600}}>\u{1F4E2} Send Telegram announcement on save</span>
              </label>
          ...
          {!annEditing?<>
            ...
            <div style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",padding:24,display:"grid",gap:16}}>
              ...
              <div style={{paddingTop:20,borderTop:"1px solid var(--border)"}}>
                <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:12}}><input type="checkbox" checked={!!annForm.sendTelegram} onChange={e=>{const ck=e.target.checked;setAnnForm(p=>{const msg="\u{1F4E2} "+(p.title||"Announcement")+"\n\n"+(p.body||"");return{...p,sendTelegram:ck,telegramMessage:ck?(p.telegramMessage||msg):p.telegramMessage};});}} style={{accentColor:"var(--accent)",width:18,height:18}}/><span style={{fontSize:14,fontWeight:600}}>\u{1F4E2} Send Telegram announcement on save</span></label>
