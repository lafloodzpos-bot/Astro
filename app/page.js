          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",width:"100%",maxWidth:360,maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
            ...
            <div style={{padding:14,overflowY:"auto",flex:1}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8,marginBottom:12}}>
                ...
                <Stars rating={sel.tasteRating} label="Taste"/>
                {sel.potency&&<span style={{fontSize:10,color:"var(--dim)"}}>Potency: <strong style={{color:"var(--text)"}}>{sel.potency}</strong></span>}
                {sel.strain&&<span style={{fontSize:10,color:"var(--dim)"}}>Strain: <strong style={{color:"var(--text)"}}>{sel.strain}</strong></span>}
                {sel.weight&&<span style={{fontSize:10,color:"var(--dim)"}}>Weight: <strong style={{color:"var(--text)"}}>{sel.weight}</strong></span>}
              </div>
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{addToCart(sel);setSel(null);}} style={{flex:1,padding:"11px",borderRadius:10,border:"none",background:"linear-gradient(135deg,var(--accent),#8b5cf6)",color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer"}}>Add to Cart</button>
        ...
        {page==="shop"&&!loading&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:16}}>
            {filtered.map((product,i)=>{
              const inS=product.inStock!==false;
              return(<div key={product.id} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",overflow:"hidden",cursor:"pointer",opacity:inS?1:0.55}} onClick={()=>setSel(product)}>
                ...
                <div style={{padding:"14px 16px 16px"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();inS&&addToCart(product);}} style={{padding:"8px 16px",borderRadius:10,border:"none",background:inS?"linear-gradient(135deg,var(--accent),#8b5cf6)":"var(--border)",color:"#fff",fontSize:12,fontWeight:600,cursor:inS?"pointer":"default"}}>
                      {!inS?"Sold Out":addedId===product.id?"Added!":"Add to Cart"}
                    </button>
                  </div>
                </div>
              </div>);
          ...
          :<>
            <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:24}}>
              {cart.map(item=>(<div key={item.id} style={{display:"flex",alignItems:"center",gap:14,background:"var(--card)",borderRadius:14,padding:"14px 18px",border:"1px solid var(--border)",flexWrap:"wrap"}}>
                <div style={{width:48,height:48,borderRadius:10,overflow:"hidden",background:"var(--surface)",flexShrink:0}}>
                  {item.image?<img src={item.image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",fontSize:10,color:"var(--dim)"}}>-</div>}
                </div>
            ...
            <div style={{background:"var(--card)",borderRadius:14,padding:20,border:"1px solid var(--border)",marginBottom:16}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {SHIPPING_OPTIONS.map(opt=>(<label key={opt.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",borderRadius:10,border:"1px solid "+(shipping===opt.id?"var(--accent)":"var(--border)"),background:shipping===opt.id?"rgba(108,92,231,.08)":"transparent",cursor:"pointer"}}>
                  <input type="radio" name="ship" value={opt.id} checked={shipping===opt.id} onChange={()=>setShipping(opt.id)} style={{accentColor:"var(--accent)"}}/>
            ...
            <div style={{background:"var(--card)",borderRadius:14,padding:24,border:"1px solid var(--border)",marginBottom:20}}>
              ...
              <div style={{borderTop:"1px solid var(--border)",paddingTop:14,marginTop:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <span style={{fontFamily:"'Outfit'",fontSize:26,fontWeight:800,color:"var(--accent)"}}>{fmt(tot)}</span>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <button onClick={copyOrd} style={{width:"100%",padding:"16px",borderRadius:14,border:"none",background:copied?"linear-gradient(135deg,var(--green),#16a34a)":"linear-gradient(135deg,var(--accent),#8b5cf6)",color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",transition:"all .3s"}}>
                {copied?"Order Copied!":"Copy Order Details"}
              </button>
              <button onClick={()=>{window.open("https://t.me/astrolosangeles?text="+encodeURIComponent(orderText()),"_blank");}} style={{width:"100%",padding:"16px",borderRadius:14,border:"none",background:"linear-gradient(135deg,#0088cc,#0077b5)",color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
