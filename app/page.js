...
function Stars({ rating, label }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11 }}>
      ...
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} style={{ color: i <= n ? "var(--gold)" : "var(--border)" }}>Ã¢ÂÂ</span>
      ))}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <main style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 24px 80px" }}>
        {loading && (<div style={{ textAlign: "center", padding: 80, color: "var(--muted)" }}><div style={{ fontSize: 40, marginBottom: 16 }}>Ã¢ÂÂ³</div><p>Loading products...</p></div>)}
            ...
            <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: 20, border: "1px solid var(--border)", maxWidth: 600, width: "100%", maxHeight: "90vh", overflow: "auto" }}>
              ...
              <div style={{ padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 8 }}>
                  <div>
                    <p style={{ fontSize: 11, color: "var(--dim)", fontWeight: 600, letterSpacing: ".08em" }}>SKU: {selectedProduct.sku} ÃÂ· {selectedProduct.category}</p>
        ...
        {page === "shop" && !loading && (
          <>
            ...
            {products.length === 0 && (<div style={{ textAlign: "center", padding: 60, color: "var(--muted)" }}><p style={{ fontSize: 40, marginBottom: 12 }}>Ã°ÂÂÂª</p><p>No products listed yet. Check back soon!</p></div>)}
                ...
                return (
                  <div key={product.id} style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden", transition: "all .25s ease", cursor: "pointer", opacity: inStock ? 1 : 0.55, animation: `fadeUp .4s ease both ${i * 0.06}s` }} onClick={() => setSelectedProduct(product)}>
                    ...
                    <div style={{ padding: "16px 18px 18px" }}>
                      <p style={{ fontSize: 10, color: "var(--dim)", fontWeight: 600, letterSpacing: ".08em", marginBottom: 4 }}>SKU: {product.sku} ÃÂ· {product.category}</p>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); inStock && addToCart(product); }} style={{ padding: "9px 20px", borderRadius: 10, border: "none", background: inStock ? "linear-gradient(135deg, var(--accent), #8b5cf6)" : "var(--border)", color: "#fff", fontSize: 13, fontWeight: 600, cursor: inStock ? "pointer" : "default", boxShadow: inStock ? "0 4px 16px var(--accent-glow)" : "none", transition: "all .15s" }}>
                          {!inStock ? "Sold Out" : addedId === product.id ? "Ã¢ÂÂ Added!" : "Add to Cart"}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        {page === "cart" && (
          <div style={{ maxWidth: 800, margin: "0 auto" }}>
            <button onClick={() => setPage("shop")} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, marginBottom: 20 }}>Ã¢ÂÂ Back to Shop</button>
            ...
            {cart.length === 0 ? (
              ...
              <>
                  ...
                  {cart.map((item, i) => (
                    <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 16, background: "var(--card)", borderRadius: 14, padding: "16px 20px", border: "1px solid var(--border)", flexWrap: "wrap", animation: `slideIn .3s ease both ${i * 0.05}s` }}>
                      <div style={{ width: 48, height: 48, borderRadius: 10, overflow: "hidden", background: "var(--surface)", flexShrink: 0 }}>
                        {item.image ? <img src={item.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} /> : <span style={{ fontSize: 24, display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>Ã°ÂÂÂ¦</span>}
                      </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <button onClick={() => updateQty(item.id, -1)} style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>Ã¢ÂÂ</button>
                      ...
                      <button onClick={() => removeFromCart(item.id)} style={{ background: "transparent", border: "none", color: "var(--dim)", cursor: "pointer", padding: 4 }}>Ã¢ÂÂ</button>
                    </div>
                  ))}
                </div>
                ...
                <button onClick={copyOrder} style={{ width: "100%", padding: "16px 24px", borderRadius: 14, border: "none", background: copied ? "linear-gradient(135deg, var(--green), #16a34a)" : "linear-gradient(135deg, var(--accent), #8b5cf6)", color: "#fff", fontSize: 16, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all .3s ease" }}>
                  {copied ? "Ã¢ÂÂ Order Copied to Clipboard!" : "Ã°ÂÂÂ Copy Order Details"}
                </button>
                {copied && (<p style={{ textAlign: "center", marginTop: 12, fontSize: 13, color: "var(--green)", animation: "fadeUp .3s ease" }}>Ã¢ÂÂ Paste into Telegram or Signal to submit your order</p>)}
      ...
      <footer style={{ borderTop: "1px solid var(--border)", padding: 24, textAlign: "center", fontSize: 12, color: "var(--dim)" }}>
        ÃÂ© {new Date().getFullYear()} {SITE_NAME} ÃÂ· All rights reserved
      </footer>
    </div>
  );
}
