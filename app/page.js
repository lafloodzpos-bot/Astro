...
export default function StoreFront() {
  ...
  const inputStyle = {
    ...
    background: "var(--surface)", color: "var(--text)", fontSize: 14, outline: "none", width: "100%",
  };
  ...
  return (
    <div>
      ...
      <header style={{
        ...
        background: "rgba(10,10,15,.88)", backdropFilter: "blur(20px)",
        WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid var(--border)", padding: "0 24px",
      }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => { setPage("shop"); setSelectedProduct(null); }}>
            ...
            <span style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 20 }}>
              Astro<span style={{ color: "var(--accent)" }}>Packs</span>
            </span>
          </div>
          <nav style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {["shop", "cart"].map((p) => (
              <button key={p} onClick={() => { setPage(p); setSelectedProduct(null); }} style={{
                background: page === p ? "var(--card)" : "transparent",
                border: page === p ? "1px solid var(--border)" : "1px solid transparent",
                color: page === p ? "var(--text)" : "var(--muted)",
                padding: "7px 18px", borderRadius: 8, cursor: "pointer", fontSize: 14, fontWeight: 500, textTransform: "capitalize",
              }}>
          ...
          <button onClick={() => setPage("cart")} style={{ background: "transparent", border: "none", color: "var(--text)", cursor: "pointer", padding: 6, position: "relative" }}>
            ...
            {cartCount > 0 && (
              <span style={{ position: "absolute", top: -2, right: -6, background: "var(--red)", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: "50%", width: 20, height: 20, display: "flex", alignItems: "center", justifyContent: "center" }}>{cartCount}</span>
            )}
          </button>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 24px 80px" }}>
            ...
            <div onClick={(e) => e.stopPropagation()} style={{
              ...
              <div style={{ padding: 24 }}>
                ...
                {selectedProduct.description && (
                  <p style={{ color: "var(--muted)", fontSize: 14, lineHeight: 1.6, marginTop: 12, marginBottom: 16 }}>{selectedProduct.description}</p>
                )}
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginBottom: 16 }}>
                  ...
                  {selectedProduct.potency && (
                    <span style={{ fontSize: 11, color: "var(--dim)" }}>Potency: <strong style={{ color: "var(--text)" }}>{selectedProduct.potency}</strong></span>
                  )}
                  {selectedProduct.strain && (
                    <span style={{ fontSize: 11, color: "var(--dim)" }}>Strain: <strong style={{ color: "var(--text)" }}>{selectedProduct.strain}</strong></span>
                  )}
                  {selectedProduct.weight && (
                    <span style={{ fontSize: 11, color: "var(--dim)" }}>Weight: <strong style={{ color: "var(--text)" }}>{selectedProduct.weight}</strong></span>
                  )}
                </div>

                <div style={{ display: "flex", gap: 12 }}>
                  <button onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }}
                    style={{ flex: 1, padding: "14px 24px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, var(--accent), #8b5cf6)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>
                    Add to Cart
                  </button>
                  <button onClick={() => setSelectedProduct(null)}
                    style={{ padding: "14px 24px", borderRadius: 12, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--muted)", fontSize: 15, cursor: "pointer" }}>
                    Close
                  </button>
                </div>
              </div>
              ...
              {filtered.map((product, i) => {
                ...
                return (
                  <div key={product.id} style={{
                    ...
                    animation: `fadeUp .4s ease both ${i * 0.06}s`,
                  }}
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      {product.badge && (
                        <span style={{
                          ...
                          background: product.badge === "HOT" ? "var(--red)" : product.badge === "NEW" ? "var(--accent)" : product.badge === "SALE" ? "var(--green)" : "var(--gold)",
                          color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 6,
                        }}>{product.badge}</span>
                      )}
                    </div>
                    <div style={{ padding: "16px 18px 18px" }}>
                      <p style={{ fontSize: 10, color: "var(--dim)", fontWeight: 600, letterSpacing: ".08em", marginBottom: 4 }}>SKU: {product.sku} · {product.category}</p>
                      <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 6, lineHeight: 1.3, minHeight: 40 }}>{product.name}</h3>
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                        <Stars rating={product.smellRating} label="Smell" />
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <span style={{ fontFamily: "'Outfit'", fontSize: 22, fontWeight: 700, color: "var(--accent)" }}>{fmt(product.price)}</span>
                        <button onClick={(e) => { e.stopPropagation(); inStock && addToCart(product); }}
                          style={{
                            padding: "9px 20px", borderRadius: 10, border: "none",
                            background: inStock ? "linear-gradient(135deg, var(--accent), #8b5cf6)" : "var(--border)",
                            color: "#fff", fontSize: 13, fontWeight: 600, cursor: inStock ? "pointer" : "default",
                            boxShadow: inStock ? "0 4px 16px var(--accent-glow)" : "none", transition: "all .15s",
                          }}>
        ...
        {page === "cart" && (
          <div style={{ maxWidth: 800, margin: "0 auto" }}>
            <button onClick={() => setPage("shop")} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 14, marginBottom: 20 }}>
              ← Back to Shop
            </button>
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
                  {cart.map((item, i) => (
                    <div key={item.id} style={{
                      ...
                      animation: `slideIn .3s ease both ${i * 0.05}s`,
                    }}>
                      <div style={{ width: 48, height: 48, borderRadius: 10, overflow: "hidden", background: "var(--surface)", flexShrink: 0 }}>
                        {item.image ? <img src={item.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} /> : <span style={{ fontSize: 24, display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>📦</span>}
                      </div>
                      ...
                      <button onClick={() => removeFromCart(item.id)} style={{ background: "transparent", border: "none", color: "var(--dim)", cursor: "pointer", padding: 4 }}>✕</button>
                    </div>
                  ))}
                </div>
                ...
                <button onClick={copyOrder} style={{
                  ...
                  background: copied ? "linear-gradient(135deg, var(--green), #16a34a)" : "linear-gradient(135deg, var(--accent), #8b5cf6)",
                ...
                {copied && (
                  <p style={{ textAlign: "center", marginTop: 12, fontSize: 13, color: "var(--green)", animation: "fadeUp .3s ease" }}>
                    ✓ Paste into Telegram or Signal to submit your order
                  </p>
                )}
