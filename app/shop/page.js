      ...
      <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(10,10,15,.88)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid var(--border)",padding:"0 24px"}}>
        <div style={{maxWidth:1280,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:64}}>
          <div style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>{setPage("shop");setSel(null);}}>
            <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,var(--accent),#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff"}}>A</div>
        ...
        {sel&&<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:200,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setSel(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",width:"100%",maxWidth:360,maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
            ...
            <div style={{padding:14,overflowY:"auto",flex:1}}>
              <div style={{display:"flex",gap:8,marginBottom:10}}>{sel.image&&<a href={sel.image} download style={{display:"flex",alignItems:"center",gap:5,padding:"6px 12px",borderRadius:8,background:"var(--surface)",border:"1px solid var(--border)",color:"var(--muted)",fontSize:11,fontWeight:600,textDecoration:"none",cursor:"pointer"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Save Photo</a>}{sel.video&&<a href={sel.video} download style={{display:"flex",alignItems:"center",gap:5,padding:"6px 12px",borderRadius:8,background:"var(--surface)",border:"1px solid var(--border)",color:"var(--muted)",fontSize:11,fontWeight:600,textDecoration:"none",cursor:"pointer"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Save Video</a>}</div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                {sel.salePrice?<><span style={{fontFamily:"'Outfit'",fontSize:14,textDecoration:"line-through",color:"var(--dim)"}}>{fmt(sel.price)}</span><span style={{fontFamily:"'Outfit'",fontSize:20,fontWeight:800,color:"var(--red)"}}>{fmt(sel.salePrice)}</span></>:<span style={{fontFamily:"'Outfit'",fontSize:20,fontWeight:800,color:"var(--accent)"}}>{fmt(sel.price)}</span>}
              ...
              <div style={{display:"flex",gap:10}}>
                ...
                {tgLink&&<button onClick={()=>{const price=sel.salePrice||sel.price;const msg="Interested in "+sel.name+" - $"+price;window.open(tgLink+"?text="+encodeURIComponent(msg),"_blank");}} style={{padding:"11px 16px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#0088cc,#0077b5)",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}><svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>Ask</button>}
        ...
        {page==="shop"&&!loading&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:16}}>
            {filtered.map((product)=>{const inS=product.inStock!==false;return(<div key={product.id} style={{background:"var(--card)",borderRadius:16,border:"1px solid var(--border)",overflow:"hidden",cursor:"pointer",opacity:inS?1:0.55}} onClick={()=>setSel(product)}>
              ...
              <div style={{padding:"14px 16px 16px"}}><p style={{fontSize:10,color:"var(--dim)",fontWeight:600,letterSpacing:".06em",marginBottom:3}}>SKU: {product.sku}</p><h3 style={{fontSize:14,fontWeight:600,marginBottom:4,lineHeight:1.3,minHeight:36}}>{product.name}</h3>
                {product.noDiscount&&<p style={{fontSize:9,color:"var(--dim)",marginBottom:4}}>No additional discounts</p>}
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontFamily:"'Outfit'",fontWeight:700,color:"var(--accent)"}}>{product.salePrice?<><span style={{fontSize:14,textDecoration:"line-through",color:"var(--dim)",marginRight:4}}>{fmt(product.price)}</span><span style={{fontSize:20,color:"var(--red)"}}>{fmt(product.salePrice)}</span></>:<span style={{fontSize:20}}>{fmt(product.price)}</span>}</span>
