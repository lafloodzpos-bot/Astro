...
export async function POST(request) {
  ...
  if (body.action === "change_password") { if (!body.newPassword || body.newPassword.length < 6) return NextResponse.json({ error: "Min 6 chars" }, { status: 400 }); await kv.set(P + "admin_password", body.newPassword); return NextResponse.json({ success: true }); }
  ...
  if (body.action === "log_admin") { const al = (await kv.get(P + "admin_logs")) || []; al.unshift({ ip, date: new Date().toISOString() }); if (al.length > 200) al.length = 200; await kv.set(P + "admin_logs", al); return NextResponse.json({ success: true }); }
